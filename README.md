...
// visit all visible cells from the origin and up the given radius
a.visitFieldOfView(map, originX, originY, radius);
```

The method `IFovAlgorithm#visitFieldOfView` is guaranteed to call `IBoard#visit` **only once**, and always before
...
a.visitConeFieldOfView(map, originX, originY, radius, 0, 180);
```
...
If you only need to **test** for line of sight, set the last argument, `calculateProject`, to `false`.
...
List<Point> path;
if (a.existsLineOfSight(map, startX, startY, endX, endY, true))
  path = a.getProjectPath();
else
  // do something else?
```

Note that if a line of sight cannot be established, the path will be `null`.
